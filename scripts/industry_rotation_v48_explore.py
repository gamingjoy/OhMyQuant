"""行业轮动策略 v48 探索：PE+PB双估值调节（PB权重减半）

设计目的：
  v47 PE+PB双估值（各0.1）结果：
    - IS Sharpe +0.0657 (大幅提升) ✓
    - 2024 Sharpe +0.1041 (改善) ✓
    - 2018-2021 Sharpe -0.2157 (大幅恶化) ✗ 过拟合风险

  v48 假设：B/P权重过大是跨周期恶化的主因
  调整：alpha_ep=0.15 + alpha_bp=0.05（B/P权重减半）
  预期：保留近年改善，减少跨周期恶化
...
def main():
    print("=" * 80, flush=True)
    print("v48 探索：PE+PB双估值调节（PB权重减半 ep=0.15+bp=0.05）", flush=True)
    print("=" * 80, flush=True)
    print(flush=True)
    ...
    print("【参考】v47 (ep=0.1+bp=0.1) baseline:", flush=True)
    print(
        f"  v47 IS Sharpe={v47_ref['is_sharpe']:.4f}  "
        f"2024={v47_ref['2024_sharpe']:.4f}  "
        f"2025={v47_ref['2025_sharpe']:.4f}  "
    ...
    # Part 1: v48 IS
    print(f"\n>> v48 IS (2022-2025):", flush=True)
    r_is = run_backtest("v48", "2022-01-01", "2025-12-31", label="v48 IS")

    # Part 2: 分年份
    yearly = {}
    for year in [2022, 2023, 2024, 2025]:
        print(f"\n>> v48 {year}年:", flush=True)
        r_y = run_backtest(
            "v48", f"{year}-01-01", f"{year}-12-31", label=f"v48 {year}"
        )
        yearly[str(year)] = r_y

    # Part 3: 2018-2021 跨周期
    print(f"\n>> v48 2018-2021 (跨周期验证):", flush=True)
    r_2018 = run_backtest("v48", "2018-01-01", "2021-12-31", label="v48 2018-2021")
    ...
    # Part 4: 总结
    print(f"\n\n{'='*80}", flush=True)
    print("【总结】v48 vs v43 / v47", flush=True)
    print(f"{'='*80}", flush=True)

    print(
        f"\n  {'指标':<15} {'v43':>10} {'v47':>10} {'v48':>10} {'v48-v43':>10}",
        flush=True,
    )
    ...
    for year in ["2022", "2023", "2024", "2025"]:
        ...
        print(
            ...
            f"{v48_s:>10.4f} {v48_s-v43_s:>+10.4f}",
            flush=True,
        )
    print(
    ...
    is_pass = (
        ...
        and r_2018["sharpe"] >= -0.10  # 跨周期恶化限制在-0.10以内
    )
    if is_pass:
        print(
            f"\n  ✓ v48 通过验证：2024改善({yearly['2024']['sharpe']:.4f} > {v43_ref['2024_sharpe']:.4f})，"
            f"跨周期恶化减小({r_2018['sharpe']:.4f} vs v47 {v47_ref['2018_2021_sharpe']:.4f})",
            flush=True,
        )
    else:
        print(f"\n  ✗ v48 未通过验证", flush=True)
        if yearly["2024"]["sharpe"] <= v43_ref["2024_sharpe"]:
            print(
                f"    2024未改善: v48={yearly['2024']['sharpe']:.4f} vs v43={v43_ref['2024_sharpe']:.4f}",
                flush=True,
            )
        if r_2018["sharpe"] < -0.10:
            print(
                f"    跨周期恶化仍大: v48={r_2018['sharpe']:.4f} (阈值-0.10)",
                flush=True,
            )
    ...
    output_file = output_dir / "v48_pe_pb_reduced.json"
