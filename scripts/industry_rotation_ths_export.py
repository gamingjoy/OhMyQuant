"""同花顺模拟盘交易流水导出（行业轮动策略 industry_rotation_v5）
...
文件组织:
  ...
  output/ths/industry_rotation_v5/
...
用法:
    ...
    python scripts/industry_rotation_ths_export.py

    # 指定版本和初始资金
    python scripts/industry_rotation_ths_export.py --version v5 --capital 10000000

    # 只生成指定日期的调仓文件（日常每日运行用）
    python scripts/industry_rotation_ths_export.py --date 20260715
"""
...
def load_oos_result(version: str) -> dict:
    """加载 OOS 回测结果"""
    oos_file = Path(f"output/oos_compare/industry_rotation/{version}_oos.json")
    if not oos_file.exists():
        raise FileNotFoundError(
            f"OOS 回测结果不存在: {oos_file}\n"
            f"请先运行: python scripts/industry_rotation_oos.py {version}"
        )
