"""expertForest_v1 策略 OOS 净值分析

基于同花顺实际交易文件回放建仓与调仓，
计算 2026-06-01 以来的净值表现，对比沪深300，并给出优化建议。

用法:
    python scripts/expertforest_v1/expertforest_v1_nav_analysis.py              # 默认 v1
    python scripts/expertforest_v1/expertforest_v1_nav_analysis.py --version v1
"""
...
def main():
    ...
    parser.add_argument("--version", default="v1", help="策略版本(默认 v1)")
    args = parser.parse_args()
    version = args.version

    ths_dir = Path("output/ths/expertforest_v1")
    ...
    # 8. 输出报告
    print("\n" + "=" * 70)
    print(f"  expertForest_v1 策略 {version} OOS 净值分析 ({OOS_START} → {latest_date})")
    print("=" * 70)
    print(f"  交易日数:     {n_days}")
    ...
    if excess_total > 0:
        ...
        print(f"        - expertForest_v1 信号是否失效")
        print(f"        - 大盘过滤是否过于敏感")
