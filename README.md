...
| `etf` | v3 | ICIR + 回撤防御 | ETF 动量轮动（风控增强） |
...
# 策略对比（两个结果 JSON）
omq compare output/ycj_v1/results.json output/dl_v1/results.json --report output/comparison.html

# 多策略集成（从已保存结果加载，无需重跑回测）
omq ensemble dl_v1 ycj_v1 rl_v1 etf_v3 --weighting perf_weight
omq ensemble dl_v1 etf_v3 --weighting equal

# 策略优化
omq optimize walk-forward ycj v1 --window 1Y --step 1Y
omq optimize param-search ycj v1 --params '{"top_n": [20, 50, 100]}' --n-trials 30
...
│   │   └── strategies/           #   8 个策略版本（含 etf v3 回撤防御）
│   ├── execution/                # 执行系统
