...
> **当前最终版本**: ind_v5 (mf10_mom60_120_mkt20, final)
> **锁定日期**: 2026-07-16
...
{strategy_type}_{version}  →  超参标签  →  状态标记
     ind      _    v5     →  mf10_mom60_120_mkt20  →  final
```

| 组成 | 说明 | 示例 |
|------|------|------|
| `strategy_type` | 行业轮动 | `ind` = Industry Rotation |
| `version` | 主版本号 | `v1` ~ `v8` |
...
|------|----------|----------|-----------|------------|------|----------|
| ind_v1 | mom20_60_drawdown | -6.27% | -0.0043 | -23.04% | archived | 日频+drawdown+20/60日动量 |
| ind_v2 | mom20_60_vol15 | +2.74% | 0.1402 | -30.60% | archived | 周频+vol_target0.15 |
| ind_v3 | mom60_120_mkt60 | +38.55% | 0.4532 | -24.80% | archived | 周频+大盘60日过滤+60/120日+regime |
| ind_v4 | mom60_120_mkt20_vol12 | +59.89% | 0.6094 | -23.81% | archived | 大盘10/20日敏感过滤+纯动量 |
| **ind_v5** | **mf10_mom60_120_mkt20** | **+50.63%** | **0.5766** | **-20.17%** | **final** | **10因子等权多因子选股** |
| ind_v6 | mf8_momenh_lowvol | +39.99% | 0.4885 | -21.11% | archived | 动量增强+低波因子(负权重) |
| ind_v7 | ml_lgb252_h20 | +23.92% | 0.4060 | -20.25% | archived | ML选股(LightGBM,原始特征) |
| ind_v8 | ml_lgb504_zs_h20 | +31.64% | 0.4733 | -23.70% | archived | ML增强(z-score+504天窗口+300树) |
...
```bash
python scripts/ind_is.py v5
```

### 7.4 运行 OOS 回测

```bash
python scripts/ind_oos.py v5
```

### 7.5 版本对比

```bash
python scripts/ind_is.py v4 v5     # IS对比
python scripts/ind_oos.py v4 v5    # OOS对比
```
