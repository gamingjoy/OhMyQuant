"""行业轮动策略 v26（个股逆波动率加权，结构性改进）—— [ITER]

状态: iter（试验中，2026-07-20）
baseline: v23 (IS Sharpe 0.4476 / OOS Sharpe 2.4951 / OOS +5.39%)

v26 = v23 + use_inv_vol_weight=True（逆波动率加权）

设计目的：
  v23 个股等权配置（每只0.10），忽略个股风险差异
  v26 改用逆波动率加权（Risk Parity思想）
  weight_i ∝ 1/vol_i，低波动股票权重高，高波动股票权重低
  理论依据：降低组合整体波动，提升风险调整后收益

理论依据：
  - Risk Parity: 各资产对组合风险的贡献相等
  - 简化版：权重与波动率倒数成正比
  - 学术研究（Maillard et al. 2010）证明逆波动率加权在期望收益-风险平面上占优
  - 与等权相比：减少高波动股的暴露，平滑收益曲线

v26 改进：
  - use_inv_vol_weight: False → True
  - inv_vol_window: 20（默认）
  - 跳过 max_stock_weight 上限（避免破坏风险平价）
  - 其他配置同 v23

v25验证失败：加cash_earnings_to_price_ratio无效（IS-21.8%，OOS不变）
转向结构性改进而非因子扩展
...
@register_strategy("industry_rotation", "v26")
class IndustryRotationStrategyV26(BaseStrategy):
    """行业轮动策略 industry_rotation_v26 (multiperiod_rrg_pe_top3_stocks3_invvol, iter)
    ...
    ) -> "IndustryRotationStrategyV26":
        if strategy_type != "industry_rotation" or version != "v26":
            raise ValueError(f"不支持的策略版本: {strategy_type} {version}")

        base_config = {
            "strategy_type": "industry_rotation",
            "strategy_version": "v26",
            "strategy_name": "行业轮动策略 industry_rotation_v26 (multiperiod_rrg_pe_top3_stocks3_invvol, iter)",
            "description": "多周期RRG+PE过滤+Top-3行业3股+逆波动率加权:12因子+沪深300 [iter]",
            "backtest": {
            ...
            "selection": {
                ...
                "industry_rotation": {
                    ...
                    "use_inv_vol_weight": True,  # NEW in v26: 启用逆波动率加权
                    "inv_vol_window": 20,
                    "use_rrg": True,
                    "rs_ratio_window": 220,
