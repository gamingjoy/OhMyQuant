"""行业轮动策略 v36（v30基础上关闭行业短期风险过滤）—— [iter]

状态: iter（试验中，2026-07-20）
baseline: v30 (IS 0.4249 / OOS 2.6787 / OOS +6.66%)

v36 = v30 + industry_risk_filter: True → False

设计目的：
  v30使用industry_risk_filter（20日动量过滤，剔除下跌行业）
  v34发现该过滤将top_industries=4限制为3个
  v36关闭该过滤，验证是否能选到更多行业、提升分散度
"""
...
@register_strategy("industry_rotation", "v36")
class IndustryRotationStrategyV36(BaseStrategy):
    """行业轮动策略 industry_rotation_v36 (v30_no_risk_filter, iter)"""

    @classmethod
    def from_version(
        cls, strategy_type: str, version: str, config: dict | None = None
    ) -> "IndustryRotationStrategyV36":
        if strategy_type != "industry_rotation" or version != "v36":
            raise ValueError(f"不支持的策略版本: {strategy_type} {version}")

        base_config = {
            "strategy_type": "industry_rotation",
            "strategy_version": "v36",
            "strategy_name": "行业轮动策略 industry_rotation_v36 (v30_no_risk_filter, iter)",
            "description": "v30+industry_risk_filter=False 关闭行业短期风险过滤 [iter]",
            "backtest": {
            ...
            "selection": {
                ...
                "industry_rotation": {
                    ...
                    "market_ma_short": 10,
                    "market_ma_long": 30,
                    "industry_risk_filter": False,  # NEW in v36: True → False
