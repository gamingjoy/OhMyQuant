"""行业轮动策略 v32（v30基础上更慢RS动量：[20,60,120]）—— [iter]

状态: iter（试验中，2026-07-20）
baseline: v30 (IS 0.4249 / OOS 2.6787 / OOS +6.66%)

v32 = v30 + rs_momentum_windows: [10,30,60] → [20,60,120]

设计目的：
  v30验证了更慢的market_ma(10/30)能提升OOS表现
  v32在v30基础上放慢RS动量窗口，验证：
    - 更慢的RS动量是否能减少行业轮动的频繁切换
    - 但可能反应过慢错过行业切换信号
"""
...
@register_strategy("industry_rotation", "v32")
class IndustryRotationStrategyV32(BaseStrategy):
    """行业轮动策略 industry_rotation_v32 (slow_rrg_v30_base, iter)"""

    @classmethod
    def from_version(
        cls, strategy_type: str, version: str, config: dict | None = None
    ) -> "IndustryRotationStrategyV32":
        if strategy_type != "industry_rotation" or version != "v32":
            raise ValueError(f"不支持的策略版本: {strategy_type} {version}")

        base_config = {
            "strategy_type": "industry_rotation",
            "strategy_version": "v32",
            "strategy_name": "行业轮动策略 industry_rotation_v32 (v30_slow_rrg_20_60_120, iter)",
            "description": "v30+rs_momentum_windows=[20,60,120] 更慢RS动量 [iter]",
            "backtest": {
            ...
            "selection": {
                ...
                "industry_rotation": {
                    ...
                    "market_ma_short": 10,  # v30 base
                    "market_ma_long": 30,   # v30 base
                    ...
                    "rs_momentum_windows": [20, 60, 120],  # NEW in v32: [10,30,60] → [20,60,120]
