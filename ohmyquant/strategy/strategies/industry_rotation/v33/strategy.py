...
@register_strategy("industry_rotation", "v33")
class IndustryRotationStrategyV33(BaseStrategy):
    """行业轮动策略 industry_rotation_v33 (v30_wed_rebalance, iter)"""

    @classmethod
    def from_version(
        cls, strategy_type: str, version: str, config: dict | None = None
    ) -> "IndustryRotationStrategyV33":
        if strategy_type != "industry_rotation" or version != "v33":
            raise ValueError(f"不支持的策略版本: {strategy_type} {version}")

        base_config = {
            "strategy_type": "industry_rotation",
            "strategy_version": "v33",
            "strategy_name": "行业轮动策略 industry_rotation_v33 (v30_wed_rebalance, iter)",
            "description": "v30+weekday=2 周三调仓 [iter]",
            "backtest": {
