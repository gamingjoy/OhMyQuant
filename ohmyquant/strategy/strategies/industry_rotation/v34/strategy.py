"""行业轮动策略 v34（v30基础上top_industries=4更分散）—— [iter]

状态: iter（试验中，2026-07-20）
baseline: v30 (IS 0.4249 / OOS 2.6787 / OOS +6.66%)

v34 = v30 + top_industries: 3 → 4 + max_industry_weight: 0.30 → 0.25

设计目的：
  v30使用top_industries=3（3行业×3股票=9只）
  v34增加行业数到4（4行业×3股票=12只），更分散
  验证"更多行业分散是否能降低风险、提升收益"
"""
...
@register_strategy("industry_rotation", "v34")
class IndustryRotationStrategyV34(BaseStrategy):
    """行业轮动策略 industry_rotation_v34 (v30_top4_industries, iter)"""

    @classmethod
    def from_version(
        cls, strategy_type: str, version: str, config: dict | None = None
    ) -> "IndustryRotationStrategyV34":
        if strategy_type != "industry_rotation" or version != "v34":
            raise ValueError(f"不支持的策略版本: {strategy_type} {version}")

        base_config = {
            "strategy_type": "industry_rotation",
            "strategy_version": "v34",
            "strategy_name": "行业轮动策略 industry_rotation_v34 (v30_top4_industries, iter)",
            "description": "v30+top_industries=4 更分散 [iter]",
            "backtest": {
            ...
            "selection": {
                ...
                "industry_rotation": {
                    ...
                    "top_industries": 4,  # NEW in v34: 3 → 4
                    "stocks_per_industry": 3,
                    "momentum_short": 60,
                    "momentum_long": 120,
                    "weight_short": 0.6,
                    "weight_long": 0.4,
                    "max_industry_weight": 0.25,  # NEW in v34: 0.30 → 0.25
                    "market_filter": True,
