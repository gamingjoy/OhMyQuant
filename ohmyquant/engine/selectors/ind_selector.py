...
@register_selector("ind")
class IndRotationSelector(BaseSelector):
    """行业轮动选股器
    ...
    def __init__(self, config: dict | None = None):
        ...
        ind_cfg = self.config.get("ind", {})
        self.data_root = ind_cfg.get(
            "data_root", "D:/Work/Project/download_a_share/data"
        )
        self.top_industries: int = ind_cfg.get("top_industries", 5)
        self.stocks_per_industry: int = ind_cfg.get("stocks_per_industry", 2)
        self.momentum_short: int = ind_cfg.get("momentum_short", 20)
        self.momentum_long: int = ind_cfg.get("momentum_long", 60)
        self.weight_short: float = ind_cfg.get("weight_short", 0.6)
        self.weight_long: float = ind_cfg.get("weight_long", 0.4)
        self.max_industry_weight: float = ind_cfg.get("max_industry_weight", 0.30)
        # 大盘趋势过滤
        self.market_filter: bool = ind_cfg.get("market_filter", False)
        self.market_index: str = ind_cfg.get("market_index", "000300.XSHG")
        self.market_ma_short: int = ind_cfg.get("market_ma_short", 20)
        self.market_ma_long: int = ind_cfg.get("market_ma_long", 60)
        # 多因子选股
        self.use_factors: bool = ind_cfg.get("use_factors", False)
        self.factor_names: list[str] = ind_cfg.get("factor_names", [])
        self.factor_weights: dict[str, float] = ind_cfg.get("factor_weights", {})
        # ML选股（LightGBM预测未来收益）
        self.use_ml: bool = ind_cfg.get("use_ml", False)
        self.ml_train_window: int = ind_cfg.get("ml_train_window", 252)
        self.ml_retrain_freq: int = ind_cfg.get("ml_retrain_freq", 21)
        self.ml_target_horizon: int = ind_cfg.get("ml_target_horizon", 20)
        self.ml_n_estimators: int = ind_cfg.get("ml_n_estimators", 150)
        self.ml_max_depth: int = ind_cfg.get("ml_max_depth", 3)
        self.ml_learning_rate: float = ind_cfg.get("ml_learning_rate", 0.05)
