...
运行 mlf_v7 (沪深300 + 行业配额5只/行业) OOS 回测，对比 v2/v5/v7。

v7 = v5(沪深300) + 行业配额(max_stocks_per_industry=5)
预期：保持 ML 预测质量 + 提升行业多样性，5只×4%=20%自然满足行业约束
"""
...
V6_RESULTS = Path("output/oos_2026/mlf_v6/results.json")
V7_OUTPUT_DIR = Path("output/oos_2026/mlf_v7")


def run_v7_oos() -> dict:
    print(f"\n{'='*70}")
    print(f"运行 mlf_v7 OOS 回测 ({OOS_START} ~ {OOS_END})")
    print(f"  候选池: 沪深300, 行业配额: 5只/行业, k25_w1008")
    print(f"{'='*70}")

    t0 = time.time()
    PluginRegistry.discover_builtin()
    strategy = StrategyRegistry.create("mlf", "v7")
...
def main():
    V7_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    # 1. 运行 v7
    v7_result = run_v7_oos()
    with open(V7_OUTPUT_DIR / "results.json", "w", encoding="utf-8") as f:
        json.dump(v7_result, f, indent=2, ensure_ascii=False)
    ...
    # 3. 对比
    print(f"\n\n{'='*70}")
    print("v2 vs v5 vs v7 对比")
    print(f"{'='*70}")
    print(f"\n{'指标':<14} {'v2(无约束)':>14} {'v5(行业20%)':>14} {'v7(配额5)':>14}")
    print(f"{'-'*58}")
    print(f"{'候选池':<14} {'沪深300':>14} {'沪深300':>14} {'沪深300':>14}")
    print(f"{'行业约束':<14} {'无':>14} {'20%(振荡)':>14} {'配额5+20%':>14}")
    print(f"{'总收益':<14} {v2['total_return']*100:>13.2f}% {v5['total_return']*100 if v5 else 0:>13.2f}% {v7_result['total_return']*100:>13.2f}%")
    print(f"{'Sharpe':<14} {v2['sharpe_ratio']:>14.4f} {v5['sharpe_ratio'] if v5 else 0:>14.4f} {v7_result['sharpe_ratio']:>14.4f}")
    print(f"{'最大回撤':<14} {v2['max_drawdown']*100:>13.2f}% {v5['max_drawdown']*100 if v5 else 0:>13.2f}% {v7_result['max_drawdown']*100:>13.2f}%")
    ...
    for label, data in [("v2", v2), ("v5", v5), ("v7", v7_result)]:
        if data is None:
            continue
    ...
    print("v7 持仓明细")
    print(f"{'='*70}")
    for entry in v7_result.get("rebalance_log", []):
        date = entry["date"]
        holdings = entry.get("holdings", {})
    ...
    v7_rebal = v7_result.get("rebalance_log", [])
    if len(v7_rebal) >= 2:
        h1 = set(v7_rebal[0].get("holdings", {}).keys())
        h2 = set(v7_rebal[1].get("holdings", {}).keys())
        added = h2 - h1
        removed = h1 - h2
        print(f"\n  v7 换手 (6/1 → 7/1):")
