"""行业轮动策略 v7（ML选股+LightGBM预测收益）

v5 问题：10因子等权线性组合，无法捕捉因子间非线性关系
v6 问题：手动调权重全面恶化，说明线性加权难以优化
v7 改进：
  - 用LightGBM回归模型学习因子→未来收益的非线性关系
  - 滚动训练（252天窗口，每21天重训练）
  - 预测20日未来收益作为个股评分
  - 因子集沿用v5的10因子（已验证有效）
  - 行业层保持动量排名（已验证有效）
...
@register_strategy("ind", "v7")
class IndRotationStrategyV7(BaseStrategy):
    """行业轮动策略 v7：ML选股 (ml_lgb252_h20_csi300_mkt20)"""

    @classmethod
    def from_version(
        cls, strategy_type: str, version: str, config: dict | None = None
    ) -> "IndRotationStrategyV7":
        if strategy_type != "ind" or version != "v7":
            raise ValueError(f"不支持的策略版本: {strategy_type} {version}")

        base_config = {
            "strategy_type": "ind",
            "strategy_version": "v7",
            "strategy_name": "行业轮动策略 v7",
            "description": "ML选股：60+120日行业动量+LightGBM预测收益(10因子),大盘20日过滤",
            "backtest": {
